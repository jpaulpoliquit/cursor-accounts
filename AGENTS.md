...
MultiCursor is a native macOS 14+ `LSUIElement` menu-bar app. The user-facing name is MultiCursor. Bundle ID and Keychain service stay `app.cursorbar`. Do not change those; it locks the author out of saved seats.

Internal Swift modules and types may stay `CursorBar*`. The Xcode scheme stays `CursorBar`. The built product is `MultiCursor.app`.
...
| Script | Role |
| --- | --- |
| `Scripts/install.sh` | xcodegen if needed, Release xcodebuild, `ditto` into `/Applications` (or `$INSTALL_DIR`), `xattr -cr`. After a successful run, `/Applications/MultiCursor.app` exists. Does not leave the user in DerivedData. |
| `Scripts/package-dmg.sh` | Share path for strangers. Builds Release and writes `dist/MultiCursor-0.1.0.dmg` with the app plus an `/Applications` symlink. |
