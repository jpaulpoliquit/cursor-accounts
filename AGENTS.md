...
Internal Swift modules and types may stay `CursorBar*`. The Xcode scheme stays `CursorBar`. The built product is `Cursor Accounts.app`.
...
```bash
xcodegen generate
xcodebuild -scheme CursorBar -destination 'platform=macOS' -configuration Release build
```
...
| Script | Role |
| --- | --- |
| `Scripts/install.sh` | xcodegen if needed, Release xcodebuild, `ditto` into `/Applications` (or `$INSTALL_DIR`), `xattr -cr`. After a successful run, `/Applications/Cursor Accounts.app` exists. Does not leave the user in DerivedData. |
| `Scripts/package-dmg.sh` | Share path for strangers. Builds Release and writes `dist/Cursor-Accounts-0.1.0.dmg` with the app plus an `/Applications` symlink. |
...
./Scripts/package-dmg.sh      # dist/Cursor-Accounts-0.1.0.dmg
```
...
User-facing name is Cursor Accounts (Finder, menu, dashboard, alerts, DMG volume). Never ship “CursorBar”, “Cursor Bar”, “MultiCursor”, or “account slot” in UI. Internal Swift modules, the `CursorBar` scheme, `~/Library/Application Support/CursorBar`, and bundle ID `app.cursorbar` stay as they are.
